...
2. Clone the **Examples** repository by typing **"git clone https://github.com/ternaustralia/pfp_examples.git Examples"** at the command prompt.  You need to include **Examples** after the repository URL so that the **pfp_examples** repository is cloned into a folder called **Examples** on your local computer.
...
The control files for each site in the examples assume a particular folder structure.  You're welcome to use any folder structure that makes sense to you but if you stick with the one described below then you won't have to change anything in the example control files in order to run them.

The screen shot below shows the default folder structure used in the example files.
